...
import pl.edu.agh.crawler.result.{CollectedResult, CrawlingFail, CrawlingResult, SingleResult}
import pl.edu.agh.crawler.task.{ComposedAuthTask, ComposedTask, Crawl, SingleTask}
...
class CrawlerTest extends FlatSpec with Matchers {
  ...
  it should "crawl with depth 1" in {
    ...
    crawler crawl SingleTask("any-url", List(Crawl(1)))
  ...
  it should "crawl with depth 3" in {
    ...
    crawler crawl SingleTask("any-url", List(Crawl(3)))
  ...
  it should "crawl single task" in {
    ...
    val task: SingleTask = SingleTask("any.url", List())

    val result: CrawlingResult = crawler crawl task
  ...
  it should "return CollectedResult for ComposedTask" in {
    ...
    val result: CrawlingResult = crawler crawl ComposedTask(List())
  ...
  it should "crawl composed tasks" in {
    ...
    val task1: SingleTask = SingleTask("first.url", List(Crawl(1)))
    val task2: SingleTask = SingleTask("second.url", List(Crawl(1)))

    val result: CrawlingResult = crawler crawl ComposedTask(List(task1, task2))
  ...
  it should "wrap any exception into CrawlFail" in {
    ...
    val result: CrawlingResult = crawler crawl ComposedTask(List(task1, SingleTask("second.url", List(Crawl(1)))))

    result match {
      case CollectedResult(results) => {
        results.head match {
          case CrawlingFail(task, cause) => {
            task shouldBe task1
